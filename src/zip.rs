...
fn unzip_file(file_path: &Path, output_dir: &Path) -> Result<()>{
    ...
    for i in 0..archive.len() {
        let mut entry = archive.by_index(i).map_err(|e| anyhow!("无法读取 zip 条目 {}: {}", i, e))?;

        let entry_path = output_dir.join(entry.mangled_name());
        if entry.is_dir() {
            fs::create_dir_all(&entry_path).map_err(|e| anyhow!("无法创建目录 {}: {}", entry_path.display(), e))?;
        } else {
            ...
            copy(&mut entry, &mut file).map_err(|e| anyhow!("无法解压文件 {}: {}", entry_path.display(), e))?;
        }
    }
...
mod tests {
    ...
    fn test_auto_unzip() {
        //zip
        auto_unzip("E:\\wengchengjian\\下载\\jdk-17.0.12_windows-x64_bin.zip", "E:\\project\\rust-project\\env\\test\\java").unwrap();
        //tar.gz
        auto_unzip("E:\\wengchengjian\\下载\\jdk-17.0.12_linux-aarch64_bin.tar.gz", "E:\\project\\rust-project\\env\\test\\java2").unwrap();
    }
}